      ...
      <div className="hidden md:flex justify-between fixed top-[50%] translate-y-[-50%] left-[364px] right-4">
        <CustomLink
          ...
          className={cn(
            ...
            prevHref && "hover:bg-sky-800"
          )}
        >
          <ChevronLeft
            className={cn(
              "w-16 h-16",
              prevHref ? "text-white" : "text-slate-700"
            )}
          />
          {prevHref && (
            <div className="hidden break-keep px-2 pb-1 rounded-sm absolute bottom-[-40px] left-1/2 translate-x-[-50%] z-50 group-hover:block text-center transition bg-sky-500">
              {prevHref?.split("/").at(-1)}
            </div>
          )}
        </CustomLink>
        <CustomLink
          ...
          className={cn(
            "relative transition group",
            nextHref && "hover:bg-sky-800"
          )}
        >
          <ChevronRight
            className={cn(
              "w-16 h-16",
              nextHref ? "text-white" : "text-slate-700"
            )}
          />
          {nextHref && (
            <div className="hidden break-keep px-2 pb-1 rounded-sm absolute bottom-[-40px] left-1/2 translate-x-[-50%] z-50 group-hover:block text-center transition bg-sky-500">
      ...
      <div className="h-20 flex md:hidden justify-between">
        <CustomLink
          ...
          className={cn(
            ...
            prevHref && "bg-sky-900 hover:bg-sky-800"
          )}
        >
          <ChevronLeft
            className={cn(
        ...
        <CustomLink
          ...
          className={cn(
            "h-full flex-1 flex items-center justify-center relative text-xl transition",
            nextHref && "bg-sky-900 hover:bg-sky-800"
          )}
        >
